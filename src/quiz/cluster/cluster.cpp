...
void Proximity(std::vector<std::vector<float>> points, int id, std::vector<bool>& processedPoints, std::vector<int>& cluster, KdTree* tree, float distanceTol)
{
  ...
  if (processedPoints[id] == false)
  {
    ...
    nearby = tree->search(points[id],3.0);
